...
\begin{itemize}
  ...
  \item \textbf{性质}：\textcolor{red}{$Z(G)\trianglelefteq G$}。
  \item \textbf{结论}：\textcolor{red}{$G/Z(G)\cong \operatorname{Inn}(G)$}。
\end{itemize}
...
\begin{itemize}
  ...
  \sum_r n_r^{2}=|G|.
  $
  \item \textbf{推论2（正则表示的约化）}：\textcolor{red}{正则表示可完全约化为所有不等价不可约表示的直和，且每个不可约表示出现次数等于其维数：}
  \\
  $
...
\end{itemize}
\noindent\textcolor{blue}{\rule{\linewidth}{1pt}}
...
\begin{itemize}
  \item \textbf{对应（定理）}：$\vec x\in\mathbb{R}^3 \leftrightarrow \hat x=\sum_{a=1}^3 x_a\sigma_a$（$\sigma_a$为Pauli矩阵）；
  $\hat x$为$2\times2$无迹厄密矩阵。
  \item \textbf{完备性/取系数}：任意$2\times2$无迹厄密矩阵都可写成$\sum_a x_a\sigma_a$，
  且\textcolor{red}{$x_a=\tfrac12\mathrm{tr}(\hat x\,\sigma_a)$}。
\end{itemize}

\section*{$SU(2)\to SO(3)$：伴随作用与覆盖映射}
\begin{itemize}
  \item \textbf{内积（迹表示）}：对$\hat x=\sum_a x_a\sigma_a,\ \hat y=\sum_a y_a\sigma_a$，有\textcolor{red}{$\tfrac12\mathrm{tr}(\hat x\hat y)=\sum_a x_ay_a=\vec x\cdot\vec y$}。
  \item \textbf{有向体积（迹表示）}：\textcolor{red}{$\mathrm{tr}(\hat x\hat y\hat z)=2i\,\vec x\cdot(\vec y\times\vec z)$}（等价写法：$\vec x\cdot(\vec y\times\vec z)=\tfrac{1}{2i}\mathrm{tr}(\hat x\hat y\hat z)$）。
...
\begin{itemize}
  ...
  \item \textbf{不变量}：若$u=\begin{pmatrix} a & b \\ -\bar b & \bar a \end{pmatrix}$（$|a|^2+|b|^2=1$），
  则在同一右陪集中$u_{11}/u_{12}=a/b$不变；
  其取值范围为$\mathbb{C}\cup\{\infty\}$（$b=0\Rightarrow \infty$，$a=0\Rightarrow 0$）。
  \item 任何一个陪集确定一个比值$a/b$，任何一个比值同样确定一个陪集；
  
  \{陪集\}= SU(2)/U(1) = \{所有可能的比值$a/b$\}=$\mathbb{C}\cup\{\infty\}=\mathbb{S}^2$
\end{itemize}
\noindent\textcolor{blue}{\rule{\linewidth}{1pt}}
...
\section*{对称性自发破缺 (SSB)}
\begin{itemize}
  \item \textbf{模空间}：物理理论可能存在多个真空/基态解，构成模空间 (moduli space) $\mathcal{M}$。
  \item \textbf{群作用}：对称性群 $G$ 对 $\mathcal{M}$ 有群作用（对称变换不改变总能量，将一个真空解变为另一个）。
  \item \textbf{破缺机制}：低能时系统选择一个特定真空 $x \in \mathcal{M}$。若 $G$ 的作用移动了真空（即 $g \cdot x \neq x$），则称对称性 $G$ 不保持真空 $x$，发生\textbf{自发破缺}。
  \item \textbf{残留子群}：$G$ 中保持 $x$ 不变的子群 $G_x \le G$（即稳定子群/迷向子群）。称对称性 $G$ 破缺到 $G_x$。
\end{itemize}

\section*{Goldstone 定理}
\begin{itemize}
  \item \textbf{真空流形结构}：通常 $G$ 对 $\mathcal{M}$ 作用是可迁的，则 $\mathcal{M} \cong G/G_x$（陪集空间）。
  \item \textbf{定理}：当\textbf{连续}对称性 $G$ 自发破缺到 $G_x$ 时，会出现无质量标量粒子，称为 \textbf{Goldstone bosons}。
  \item \textbf{数量}：粒子数 $= \dim G - \dim G_x = \dim \mathcal{M}$。
\end{itemize}

\section*{SSB 例子：墨西哥帽势}
\begin{itemize}
  \item \textbf{模型}：复标量场 $\phi$，势能 $V(\phi) = (|\phi|^2 - v)^2$。
  \item \textbf{真空流形}：$\mathcal{M} = \{\phi \in \mathbb{C} \mid |\phi| = v\}$（圆圈）。
  \item \textbf{对称性}：$U(1)$。选择真空 $\phi = v e^{i\theta}$。
  \item \textbf{结果}：$U(1) \to \{e\}$（完全破缺）。
  \item \textbf{计数}：产生 $1 = \dim U(1) - \dim \{e\}$ 个 Goldstone 玻色子。
\end{itemize}

\section*{相变与对称性破缺}
\begin{itemize}
  \item 多体系统的相变常伴随对称性破缺。
  \item \textbf{液 $\to$ 固（一级相变）}：连续平移/转动 $\to$ 离散平移/ $SO(3)$ 离散子群。
  \item \textbf{气 $\to$ 液 / Ising模型（连续相变）}：Landau 理论，临界温度时发生对称性自发破缺。
\end{itemize}
\noindent\textcolor{blue}{\rule{\linewidth}{1pt}}
...
\begin{itemize}
  ...
  \item \textbf{$SU(2)$}：在$U(\vec n,\omega)$坐标下，
  $\int_{SU(2)}[dU]\,f(U)=\frac{1}{4\pi^2}\int_0^{2\pi}d\varphi\int_0^\pi\sin\theta\,d\theta\int_0^{2\pi}\sin^2\frac{\omega}{2}\,d\omega\,f(U(\vec n,\omega))$。
...
\begin{itemize}
  \item \textbf{特征标正交}：对$SU(2)$不可约不等价表示$\mathcal R,\mathcal R'$，有：
  
  $\int_{SU(2)}[dU]\,\chi_{\mathcal R}(U)\overline{\chi_{\mathcal R'}(U)}=\delta_{\mathcal R\mathcal R'}$。
  \item \textbf{不可约判据}：$\int_{SU(2)}[dU]\ |\chi_{\mathcal R}(U)|^2=1\iff \mathcal R\ \text{不可约}$。
\end{itemize}
\noindent\textcolor{blue}{\rule{\linewidth}{1pt}}
\section*{$SU(2)$ 的表示}
\begin{itemize}
  \item \textbf{自然作用}：$SU(2)$ 在 $\mathbb{C}^2$ 上有自然线性作用。设 $U=\begin{pmatrix} a & b \\ -\bar{b} & \bar{a} \end{pmatrix} \in SU(2)$，作用于矢量 $\xi = \begin{pmatrix} z \\ w \end{pmatrix}$。
  \item \textbf{逆变换}：$U^{-1} = \begin{pmatrix} \bar{a} & -b \\ \bar{b} & a \end{pmatrix}$ 作用于 $\xi$ 产生 $\xi'' = U^{-1}\xi$。
  \item \textbf{表示空间构造}：考虑 $z, w$ 的 $n$ 次齐次多项式 $f(\xi) = \sum_{k=0}^n a_k z^{n-k} w^k$。全体此类多项式构成 $n+1$ 维复线性空间 $\mathbb{C}^{n+1}$。
  \item \textbf{诱导表示定义}：定义 $U$ 在函数 $f$ 上的线性作用为 $(Uf)(\xi) := f(U^{-1}\xi)$（即作用在宗量上）。这构成了 $SU(2)$ 的线性表示 $\mathcal{R}^{(n)}$。
  \item \textbf{基底}：选取 $n+1$ 个单项式作为基底：$\psi_k(z,w) = z^{n-k} w^k, \quad k=0, 1, \dots, n$。
\end{itemize}

\section*{$SU(2)$ 表示的实例计算}
\begin{itemize}
  \item \textbf{特例}：取绕 $z$ 轴旋转 $U = U(\vec{e}_3, \omega)$：
  
  $(U\psi_k) = \psi_k(e^{-\frac{i}{2}\omega}z, e^{+\frac{i}{2}\omega}w) = e^{\frac{i}{2}\omega(2k-n)}\psi_k$
  \item \textbf{表示矩阵}：在基底 $\{\psi_0, \psi_1, \dots, \psi_n\}$ 下，$\mathcal{R}^{(n)}(U)$ 为对角阵：
  
  $\mathcal{R}^{(n)} = \operatorname{diag}\left( e^{-\frac{i}{2}n\omega},
  \ e^{\frac{i}{2}\omega(2-n)},\ \dots,\ e^{+\frac{i}{2}n\omega} \right)$
  若基底倒序排列，则对角元次序颠倒。
\end{itemize}
\section*{$SU(2)$ 的不可约表示：标记、维数、特征标}
\begin{itemize}
  \item \textbf{标记与维数}：不可约表示由自旋$j\in\frac12\mathbb{N}$标记，记为$\mathcal R_j$，且$\dim\mathcal R_j=2j+1$。
  \item \textbf{绕$e_3$轴的对角形式}：在基$\{\psi_{jm} = z^{j−m} \omega^{j+m}\}_{m=-j}^{j}$下，$\mathcal R_j(U(\vec e_3,\omega))\psi_{jm}=e^{im\omega}\psi_{jm}$，即$\mathcal R_j(U(\vec e_3,\omega))=\mathrm{diag}(e^{im\omega})_{m=-j}^{j}$。
...
\begin{itemize}
  \item \textbf{正交归一}：$\int_{SU(2)}[dU]\ \chi_{j_1}(U)\overline{\chi_{j_2}(U)}=\delta_{j_1j_2}$；
  
  等价地$\frac{1}{\pi}\int_0^{2\pi}\sin^2\frac{\omega}{2}\ \chi_{j_1}(\omega)\overline{\chi_{j_2}(\omega)}\,d\omega=\delta_{j_1j_2}$。

  $\mathcal R_j$互不等价，且均为不可约表⽰。
...
\section*{常见表示与不变量}
\begin{itemize}
  \item \textbf{平凡/标量/单重态表示}：$j=0$，$\dim=1$，$\mathcal R_0(U)=1$。
  \item \textbf{基本表示（定义）（旋量）（二重态）}：$j=\tfrac12$，$\dim=2$；存在不变量$\epsilon_{\alpha\beta}$（$\epsilon_{12}=-\epsilon_{21}=-1$）用于缩并，给出$SU(2)$不变标量$-\epsilon_{\alpha\beta}X^\alpha Y^\beta$，并满足$\epsilon^{\alpha\beta}\epsilon_{\beta\gamma}=\delta^\alpha_\gamma$；等价地可定义反对称双线性形式$\langle X,Y\rangle:=-\epsilon_{\alpha\beta}X^\alpha Y^\beta$，并有$\langle UX,UY\rangle=\langle X,Y\rangle$（$\det U=1$）。
  \item \textbf{伴随/矢量/三重态表示与李代数}：$j=1$（$\dim=3$），可由伴随作用$\hat x\mapsto U\hat xU^{-1}$诱导$3$维变换$x'^a=R(U)_{ab}x^b$；由$\{\sigma_a\}$张成的无迹厄密空间可视为$\mathfrak{su}(2)$的表示空间（伴随表示），一般李群$G$也有$\dim G$维伴随表示$\mathfrak g$（李代数）。
\end{itemize}
\noindent\textcolor{blue}{\rule{\linewidth}{1pt}}
\section*{表示的忠实性与中心作用}
\begin{itemize}
...
\section*{SO(3) 与旋量 (Spinors)}
\begin{itemize}
  \item \textbf{SO(3) 变换}：$SO(3)$ 为特殊正交 $3\times3$ 矩阵，满足 $(R^T)_{ab}R_{bc}=\delta_{ac}$。
  \item \textbf{矢量变换}：设 $\mathbb{R}^3$ 正交归一基底为 $e_a$，矢量 $X=X^a e_a$。
  \item \textbf{分量变换}：$X^a \to X'^a = R(\vec{n},\omega)_{ab}X^b$。
  \item \textbf{基底变换}：$e'_b = R_{ab}e_a$。
\end{itemize}

\section*{自旋算符与双重身份}
\begin{itemize}
  \item \textbf{定义}：电子自旋角动量算符 $s_a = \frac{\hbar}{2}\sigma_a$（沿 $e_a$ 测量）。
  \item \textbf{矢量性质}：在坐标旋转下，$s_b$ 像矢量一样变换：$s_b \to s'_b = R_{ab}s_a$。
  \item \textbf{算符性质}：$s_b$ 可用 Pauli 矩阵与 $SU(2)$ 元 $U$ 表达，满足相似变换 $s'_b = U s_b U^{-1}$。
\end{itemize}

\section*{旋量 (Spinor) 的定义}
\begin{itemize}
  \item \textbf{变换法则}：当位置空间 $\mathbb{R}^3$ 中的矢量 $X$ 做 $SO(3)$ 转动 ($X \to RX$) 时，诱导角动量本征态 $\psi$ 做 $SU(2)$ 变换 ($\psi \to U\psi$)。
  \item \textbf{定义}：$\psi$ 相对 $X$ 称为 $SO(3)$ 旋量。
  \item \textbf{推广}：旋量是相对于位置空间 $\mathbb{R}^n$ 而言的。当 $\mathbb{R}^n$ 作 $SO(n)$ 转动，$\psi$ 跟随做 $\operatorname{Spin}(n)=\widetilde{SO(n)}$ 变换。
\end{itemize}

\section*{2$\pi$ 旋转问题}
\begin{itemize}
  \item \textbf{矢量}：当转角 $\omega$ 从 $0$ 抵至 $2\pi$ 时，$X$ 转一圈回到原值 $X$。
  \item \textbf{旋量}：对任意轴 $\vec{n}$，$U(\vec{n}, 2\pi) = -\mathbf{1}_{2\times 2}$。
  \item \textbf{结果}：$\psi \to -\psi$。旋量需要转两圈 ($4\pi$) 才能回到原值。
\end{itemize}
\noindent\textcolor{blue}{\rule{\linewidth}{1pt}}
\section*{表示的直和}
\begin{itemize}
...
% \end{itemize}
\noindent\textcolor{blue}{\rule{\linewidth}{1pt}}
\section*{$SU(2)$ 不可约表示与杨图}
\begin{itemize}
  \item \textbf{杨图定义}：用盒子排列标记不可约表示。
  \item \textbf{合法性}：对于 $SU(2)$，杨图最多只有两行，且上行盒子数 $\ge$ 下行 ($Y_1 \ge Y_2 \ge 0$)。
  \item \textbf{满减规则}：两行对齐的列（即高度为2的列）对应平凡表示，可以删掉。
  \item \textbf{表示与自旋}：若删去满列后，第一行剩余 $n$ 个盒子，则对应：
  \textbf{自旋}：$j = n/2$;\textbf{维数}：$\dim = n+1 = 2j+1$
\end{itemize}
\section*{杨图维数计算：钩长公式}
\begin{itemize}
  \item \textbf{分子（内容数）}：
  将 $N$ 填入左上角盒子。同一行向右每次 $+1$，同一列向下每次 $-1$。即第 $i$ 行 $j$ 列的值为 $C_{ij} = N + (j-i)$。
  \item \textbf{分母（钩长 Hook Length）}：
  每个盒子的钩长 $h_{ij}$ = 该盒子右边的盒子数 + 该盒子下边的盒子数 + 1（自身）。
  \item \textbf{公式}：
  $\dim = \prod_{\square} \frac{Number of \square }{\text{hook length}(\square)}$
\end{itemize}
\section*{杨图计算维数公式}
对于 $SU(2)$ 的杨图（行长 $Y_1, Y_2$）：
$\dim = Y_1 - Y_2 + 1$
这与满减规则一致：$n = Y_1 - Y_2$。

\section*{张量积的杨图分解}
\textbf{规则（Littlewood-Richardson）}：
\begin{enumerate}
    \item 将第二个杨图（通常取行数少的）的盒子标号（如第一行标1）。
    \item 将这些盒子逐个添加到第一个杨图上。
    \item \textbf{约束}：形成的新图形必须是合法杨图（$SU(2)$最多2行）；同一列不能出现相同的标号。
    \item \textbf{化简}：去掉所有高度为2的列。
\end{enumerate}
\noindent\textcolor{blue}{\rule{\linewidth}{1pt}}
\section*{特殊幺正群 $SU(N)$}
\begin{itemize}
  \item \textbf{定义}：$N \times N$ 特殊幺正矩阵构成的群。满足 $UU^{\dagger} = U^{\dagger}U = \mathbf{1}$ 且 $\det U = 1$。
  \item \textbf{维数}：$\dim SU(N) = N^2 - 1$（$N^2$个复变量受幺正及行列式约束）。
  \item \textbf{子群关系}：$SU(N) < SU(M)$ ($1 < N < M$)，但非正规子群。
  \item \textbf{商空间与几何}：$SU(N)/SU(N-1) = S^{2N-1}$。$SU(N)$ 的局部几何结构类似球面的直积：$S^{2N-1} \times S^{2N-3} \times \cdots \times S^3$。
  \item \textbf{共轭与对角化}：任意 $U \in SU(N)$ 可通过共轭变换对角化为 $U_D = \operatorname{diag}(e^{i\varphi_1}, \dots, e^{i\varphi_N})$，其中 $\varphi_i \in \mathbb{R}$ 且 $\sum_{i=1}^N \varphi_i = 0$。共轭类由 $\varphi_1, \dots, \varphi_{N-1}$ 标记。
  \item \textbf{李代数 $\mathfrak{su}(N)$}：由全体 $N \times N$ \textbf{无迹厄密}矩阵构成（即 $H^\dagger=H, \operatorname{tr}H=0$）。
  \item \textbf{对易封闭性}：若 $H_1, H_2 \in \mathfrak{su}(N)$，则 $i[H_1, H_2]$ 也是无迹厄密矩阵（属于 $\mathfrak{su}(N)$）。
  \item \textbf{指数映射}：$\forall U \in SU(N)$，存在无迹厄密矩阵 $H$ 使 $U = e^{iH}$；反之亦然。
\end{itemize}
\section*{$SU(N)$ 的不可约表示}
\begin{itemize}
  \item \textbf{基本表示 (Fundamental, $\mathbf{N}$)}：$SU(N)$ 对 $\mathbb{C}^N$ 的天然线性作用（即定义本身）。
  \item \textbf{反基本表示 (Anti-fundamental, $\overline{\mathbf{N}}$)}：基本表示 $\mathbf{N}$ 的复共轭。
  \item \textbf{等价性}：$N=2$ 时，$\mathbf{N} \cong \overline{\mathbf{N}}$（$\mathbf{2} \cong \overline{\mathbf{2}}$）；$N > 2$ 时，$\mathbf{N} \not\cong \overline{\mathbf{N}}$。
  \item \textbf{伴随表示 (Adjoint)}：维数为 $N^2-1$，表示空间为李代数 $\mathfrak{su}(N)$（无迹厄密矩阵）。群作用为 $\mathcal{R}_{\text{adj}}(U)H = U^{-1}HU$。
\end{itemize}

\section*{杨图 (Young Tableaux) 标记}
\begin{itemize}
  \item $SU(N)$ 的任意不可约表示可用杨图 $Y$ 标记。
  \item \textbf{规则}：行长 $Y_1 \ge Y_2 \ge \dots \ge Y_N \ge 0$；总行数不超过 $N$。
  \item \textbf{满减规则}：因 $\det U=1$，长度为 $N$ 的列（$Y_1=\dots=Y_N=1$）对应平凡因子，可去掉不改变表示。
  \item \textbf{共轭表示}：表示 $\mathcal{R}_Y$ 的复共轭 $\overline{\mathcal{R}_Y}$ 对应的杨图与原杨图互补成一个 $N$ 行矩形。
  \item \textbf{例子}：$\mathbf{1}$ (空图); $\mathbf{N}$ (单框 $\Box$); $\overline{\mathbf{N}}$ ($N-1$个框竖排); \textbf{adj} ($N-1$竖排旁加一框)。
\end{itemize}

\section*{维数公式}
\begin{itemize}
  \item 杨图 $Y$ 对应表示的维数（Weyl 维数公式）：
  \\
  $\dim \mathcal{R}_Y = \frac{\prod_{1\le A<B \le N}((Y_A - Y_B) - (A - B))}{\prod_{1\le A<B \le N}(B - A)}$
  \item $N \ge 3$ 时，存在维数相同但不等价的表示（如 $SU(3)$ 的 $\mathbf{3}$ 和 $\overline{\mathbf{3}}$）。
\end{itemize}

\section*{特征标与 Schur 函数}
\begin{itemize}
  \item \textbf{特征标}：$\chi_{\mathcal{R}_Y}(U) = \mathrm{tr}(\mathcal{R}_Y(U))$。对角化 $U$ 后仅依赖于特征值 $x_A = e^{i\varphi_A}$。
  \item \textbf{Schur 函数}：$SU(N)$ 特征标等于 Schur 函数 $s_Y$：
  \\
  $
  \chi_{\mathcal{R}_Y}(U_D) = s_Y(e^{i\varphi_1}, \dots, e^{i\varphi_N}) = \frac{\det[e^{i\varphi_A(Y_B + N - B)}]}{\det[e^{i\varphi_A(N - B)}]}
  $
  \item \textbf{Vandermonde 行列式}：分母即 $\Delta = \prod_{1\le A<B \le N} (e^{i\varphi_A} - e^{i\varphi_B})$。
  \item \textbf{性质}：$\sum \varphi_A = 0$；$s_Y(\varphi \to 0) = \dim \mathcal{R}_Y$。
\end{itemize}

\section*{$SU(N)$ 的 Haar 测度}
\begin{itemize}
  \item \textbf{类函数积分}：对只依赖特征值的类函数 $f(U)=f(\varphi_1,\dots,\varphi_N)$，群积分化为 $N-1$ 重积分：
  \\
  $
  \int_{SU(N)} [dU]f(U) \propto \int \prod_{A=1}^{N-1} d\varphi_A \, J(\varphi) \, f(\varphi)
  $
  \item \textbf{测度密度 (Jacobian)}：
  \\
  $
  J(\varphi) = \prod_{A \neq B} (e^{i\varphi_A} - e^{i\varphi_B}) = \left| \prod_{A<B} (e^{i\varphi_A} - e^{i\varphi_B}) \right|^2
  $
  \item \textbf{$N=2$ 特例}：$J(\varphi) \propto \sin^2 \varphi$。积分测度为 $\frac{1}{\pi}\int_0^{2\pi} d\omega \sin^2 \frac{\omega}{2}$（对应角参数化）。
\end{itemize}
\section*{Haar 测度与正交归一性}
\begin{itemize}
  \item \textbf{Vandermonde 行列式}：Haar 测度的雅可比行列式 $J(\varphi)$ 与 Vandermonde 行列式 $\Delta = \det[e^{i\varphi_A(N-B)}]$ 有关：$J(\varphi) = |\Delta|^2 = \prod_{A<B} |e^{i\varphi_A} - e^{i\varphi_B}|^2$。
  \item \textbf{$N=2$ 特例}：积分测度为 $\frac{1}{\pi} \int_0^{2\pi} d\omega \sin^2(\frac{\omega}{2})$（若取角参数 $\omega$）。
  \item \textbf{Schur 函数正交性}：$\int [dU] \chi_Y(U) \overline{\chi_{Y'}(U)} = \frac{1}{N!} \int \prod d\varphi_A J(\varphi) s_Y(\varphi) \overline{s_{Y'}(\varphi)} = \delta_{YY'}$。
\end{itemize}

\section*{张量积分解 (Littlewood-Richardson 规则)}
\begin{itemize}
  \item \textbf{问题}：两不可约表示的张量积通常可约：$\mathcal{R}_Y \otimes \mathcal{R}_{Y'} = \bigoplus_{\tilde{Y}} N_{Y Y'}^{\tilde{Y}} \mathcal{R}_{\tilde{Y}}$。
  \item \textbf{杨图算法步骤}：
  \item 1. \textbf{标记}：给 $Y'$ 的第 $i$ 行的所有格子填上数字 $i$。
  \item 2. \textbf{移动}：将 $Y'$ 的格子按行（第1行、第2行...）依次添加到 $Y$ 上。
  \item 3. \textbf{规则}：
    \begin{itemize}
      \item \textbf{合法性}：每步结果必须是合法的杨图（行不增，左/上对齐）。
      \item \textbf{列唯一}：同一列中不能出现相同的数字。
      \item \textbf{逆序读取（Lattice Word）}：从右往左、从上往下读取添加的数字序列，任意时刻读到的数字 $i$ 的个数必须 $\ge$ 数字 $i+1$ 的个数。
    \end{itemize}
  \item 4. \textbf{结果}：所有满足规则的最终图形即为分解后的直和项（对应不同的不可约表示）。
  \item \textbf{例子}：$SU(3)$ 中 $\mathbf{3} \otimes \overline{\mathbf{3}} = \mathbf{1} \oplus \mathbf{8}$（伴随表示）。
\end{itemize}

\section*{$SU(N)$ 的中心}
\begin{itemize}
  \item \textbf{定义}：与群中所有元素对易的元素集合 $C(SU(N)) = \{u \in SU(N) \mid \forall U, uU=Uu\}$。
  \item \textbf{推导}：由 Schur 引理，基本表示不可约 $\Rightarrow$ 中心元素必须是单位阵的倍数 $u = \lambda \mathbf{1}_{N \times N}$。
  \item \textbf{约束}：$\det u = \lambda^N = 1 \Rightarrow \lambda = e^{i \frac{2\pi k}{N}}$ ($k=0, 1, \dots, N-1$)。
  \item \textbf{结论}：$C(SU(N)) \cong \mathbb{Z}_N$（$N$ 阶循环群）。
\end{itemize}
\end{multicols*}
\end{document}
